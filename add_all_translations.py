...
# 找到 zh: 和 ms: 块的位置
zh_start = js.index('zh:')
ms_start = js.index('ms:')
ta_start = js.index('ta:' if 'ta:' in js else ms_start + 100000)

zh_block = js[zh_start:ms_start]
ms_block = js[ms_start:ta_start]

print(f'ZH block: {len(zh_block)} chars, MS block: {len(ms_block)} chars')

# 所有需要添加的键和翻译
# (key, zh_value, ms_value)
translations_to_add = [
...
for key, zh_val, ms_val in translations_to_add:
    # 检查 ZH 块是否已有此键
    zh_pattern = rf'\b{key}\s*:\s*\'[^\']*\''
    if not re.search(zh_pattern, zh_block):
        # 在 ZH 块末尾（} 之前）插入
        insert_point = zh_block.rfind('\n}')
        if insert_point > 0:
            new_entry = f"\n    {key}: '{zh_val}',"
            zh_block = zh_block[:insert_point] + new_entry + zh_block[insert_point:]
            count_zh += 1
    
    # 检查 MS 块是否已有此键
    if not re.search(zh_pattern, ms_block):  # 用同样模式检查 ms
        ms_pattern = rf'\b{key}\s*:\s*\'[^\']*\''
        if not re.search(ms_pattern, ms_block):
            insert_point = ms_block.rfind('\n}')
            if insert_point > 0:
                new_entry = f"\n    {key}: '{ms_val}',"
                ms_block = ms_block[:insert_point] + new_entry + ms_block[insert_point:]
                count_ms += 1

print(f'Added ZH: {count_zh}, MS: {count_ms}')

# 重建文件
new_js = js[:zh_start] + zh_block + js[ms_start:ms_start] + ms_block + js[ms_start+len(ms_block):]

with open(JS, 'w', encoding='utf-8') as f:
    f.write(new_js)

print(f'Saved app.js! New size: {len(new_js)}')
