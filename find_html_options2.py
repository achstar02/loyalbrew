...
# Also search for the Chinese text
...
# Search in app.js too
print('\n=== Searching app.js for Chinese stamp text ===')
with open('C:/Users/Administrator/CodeBuddy/20260416214625/app.js', 'r', encoding='utf-8') as f:
    js = f.read()
for pat in [r'每消费', r'1印章', r'免费菜单']:
    matches = list(re.finditer(pat, js))
    if matches:
        print(f'\n"{pat}": {len(matches)} matches in app.js')
        for m in matches[:3]:
            start = max(0, m.start()-40)
