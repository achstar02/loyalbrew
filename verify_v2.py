...
# 正确找到 LANGS 对象中的 ms: 块
# 策略：先找到 LANGS，然后在其中找 en/zh/ms/ta
...
print(f'EN: {en_open}-{p-1} ({p-1-en_open} chars)')
print(f'ZH: {zh_open}-{p2-1} ({p2-1-zh_open} chars)')
print(f'MS: {ms_open}-{p3-1} ({p3-1-ms_open} chars)')
...
keys_to_check = [
    ...
    'filterPending', 'preparingLbl', 'doneLbl', 'enableNotificationsBtn'
]
...
for key in keys_to_check:
    ...
    zh_val = zh_match.group(1).strip() if zh_match else 'NOT FOUND'
    ms_val = ms_match.group(1).strip() if ms_match else 'NOT FOUND'
    
    # Check ZH has Chinese chars (or is a reasonable value)
    zh_has_cn = any('\u4e00' <= c <= '\u9fff' for c in zh_val)
    # MS should be Malay text (not Tamil, not mostly Chinese)
    ...
    status = '✅' if (zh_ok and ms_ok) else '❌'
    print(f'{status} {key}')
    if not zh_ok:
