"""
Fix 1: Add 70 missing keys to LANGS (all 4 languages)
Fix 2: Add the 1 data-mi18n key to MERCHANT_LANGS
Fix 3: Validate syntax BEFORE saving
"""
import re, subprocess, sys, os

DEPLOY_DIR = r'C:\Users\Administrator\CodeBuddy\20260416214625\deploy'
JS_PATH = os.path.join(DEPLOY_DIR, 'app.js')
BAK_PATH = os.path.join(DEPLOY_DIR, 'app.js.bak')
HTML_PATH = os.path.join(DEPLOY_DIR, 'index.html')

# Read current files (strip BOM if present)
with open(JS_PATH, 'r', encoding='utf-8-sig') as f:
    js = f.read()
with open(HTML_PATH, 'r', encoding='utf-8-sig') as f:
    html = f.read()

print(f"JS: {len(js)} chars, HTML: {len(html)} chars")

# --- Step 1: Extract all data-i18n keys from HTML ---
html_keys = set(re.findall(r'data-i18n="([^"]+)"', html))
mi18n_key = set(re.findall(r'data-mi18n="([^"]+)"', html)).pop() if re.findall(r'data-mi18n="([^"]+)"', html) else None
print(f"HTML data-i18n keys: {len(html_keys)}, data-mi18n: {mi18n_key}")

# --- Step 2: Find each LANGS language block and its closing brace ---
def find_block_end(js_content, block_name, start_pos):
    """Find the closing } of a block like 'en:', 'zh:', 'ms:', 'ta:'"""
    colon = js_content.find(block_name + ': {', start_pos)
    if colon < 0:
        colon = js_content.find(block_name + ':{', start_pos)
    if colon < 0:
        return -1, -1
...
def add_keys_to_block(js_content, block_name, lang_keys, start_pos, new_keys, verbose=False):
    """Add new keys before the closing } of a language block"""
    end_pos, block_start = find_block_end(js_content, block_name, start_pos)
    if end_pos < 0:
        print(f"  WARNING: Could not find end of {block_name} block")
        return js_content, -1
    ...
    while i < end_pos:
        if js_content[i] == '{':
            ...
            depth -= 1
        elif js_content[i] == ':' and depth == 1:
            # Extract key name
            key_start = js_content.rfind(' ', block_start, i)
    ...
    if not to_add:
        print(f"  {block_name}: all {len(new_keys)} keys already exist")
        return js_content, end_pos
    
    print(f"  {block_name}: adding {len(to_add)}/{len(new_keys)} keys (existing: {len(existing)})")
    
    # Build insertion text - insert right before the closing }
    # Find the last comma before the }
    insert_pos = end_pos
    last_comma = js_content.rfind(',', block_start, end_pos)
    last_newline = js_content.rfind('\n', block_start, end_pos)
    if last_comma > last_newline:
        insert_pos = last_comma + 1
    else:
        # No comma found, just use end_pos - 1 (before the })
        insert_pos = end_pos
    
    # Build key-value lines with proper values for each language
    lines = []
    for key in sorted(to_add):
        if block_name == 'en':
            val = key  # English: use key as value (will show readable English)
        elif block_name == 'zh':
            # Chinese translations for the missing keys
            zh_vals = {
            ...
            ms_vals = {
                ...
                'card_emoji_icon': 'Ikon Emoji Kad', 'commissionHistoryTitle': 'SejarahKomisen', 'complaintInProgressBtn': 'Dalam Proses',
            ...
            ta_vals = {
                ...
                'getNotifiedWhenReady': 'தயாரானவுடன் அறிய',
                'higherShowsFirst': 'உயர் முதலில் காட்டு', 'kitchenDisplayLbl': 'சமையலறை காட்சி',
                'leaveEmptyBanner': 'பேனர் இல்லாமல் விடவும்', 'loyalbrew_brand': 'LoyalBrew',
                'mFreeItemLabel': 'இலவச பொருள்', 'mLoginHint': 'உங்கள் உறுப்பினர் கணக்கால் உள்நுழையவும்',
    ...
    new_entries = ',\n'.join(lines)
    new_js = js_content[:insert_pos] + ',\n' + new_entries + js_content[insert_pos:]
    return new_js, end_pos + len(new_entries) + 1 + (end_pos - insert_pos)

# --- Step 3: Find LANGS and add missing keys ---
langs_pos = js.find('const LANGS')
print("\n=== Adding missing keys to LANGS ===")
for lang in ['en', 'zh', 'ms', 'ta']:
    js, _ = add_keys_to_block(js, lang, html_keys, langs_pos, sorted(html_keys))

# --- Step 4: Fix MERCHANT_LANGS - add data-mi18n key ---
ml_pos = js.find('const MERCHANT_LANGS')
print(f"\n=== Fixing MERCHANT_LANGS (mi18n key: {mi18n_key}) ===")
if mi18n_key:
    for lang in ['en', 'zh', 'ms', 'ta']:
        js, _ = add_keys_to_block(js, lang, {mi18n_key}, ml_pos, [mi18n_key])

# --- Step 5: Validate BEFORE saving ---
print("\n=== Syntax validation ===")
import tempfile, subprocess, os

# Save temp copy for validation (without BOM)
...
with open(JS_PATH, 'w', encoding='utf-8') as f:
    f.write(js)
print(f"\nSaved: {len(js)} chars (no BOM)")

# --- Step 7: Copy to source too ---
src_js = r'C:\Users\Administrator\CodeBuddy\20260416214625\app.js'
with open(src_js, 'r', encoding='utf-8-sig') as f:
    src = f.read()
with open(src_js, 'w', encoding='utf-8') as f:
    f.write(js)
print(f"Also updated source: {src_js}")
