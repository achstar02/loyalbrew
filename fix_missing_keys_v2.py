"""
Fix missing i18n keys - Approach v2
Finds exact insertion points by looking at the LAST key:value pair before } in each block
"""
...
# Find LANGS and MERCHANT_LANGS positions
...
def extract_existing_keys(content, block_name, start_pos):
    """Extract all existing keys from a block"""
    end_pos, block_start = find_block_end(content, block_name, start_pos)
    if end_pos < 0:
        return set()
    existing = set()
    depth = 0
...
def add_keys(content, block_name, all_keys, start_pos, translations):
    """Add missing keys to a language block - SAFE insertion"""
    end_pos, block_start = find_block_end(content, block_name, start_pos)
    if end_pos < 0:
        print(f"  WARNING: Could not find {block_name} block")
        return content
    
    existing = extract_existing_keys(content, block_name, start_pos)
    to_add = [k for k in all_keys if k not in existing]
    if not to_add:
        print(f"  {block_name}: all {len(all_keys)} keys already exist")
        return content
    
    print(f"  {block_name}: adding {len(to_add)}/{len(all_keys)} keys")
    
    # The insert position is just before the closing }
    # We DON'T add a comma here - we'll add a comma only if the last entry doesn't have one
    # Actually, let's find the character just before the } and check if it's a comma
    
    char_before_close = content[end_pos - 1]
    
    # Build new entries
    trans = translations.get(block_name, {})
    lines = []
    for key in sorted(to_add):
        val = trans.get(key, key)  # Fall back to key name for EN and missing translations
    ...
    if char_before_close == ',':
        # Last entry has trailing comma - insert right before }
        insert_pos = end_pos
...
for lang in ['en', 'zh', 'ms', 'ta']:
    js_new = add_keys(js_new, lang, html_keys, langs_pos, TRANSLATIONS)

# Process MERCHANT_LANGS
print("\n=== MERCHANT_LANGS ===")
if mi18n_key:
    for lang in ['en', 'zh', 'ms', 'ta']:
        js_new = add_keys(js_new, lang, {mi18n_key}, ml_pos, TRANSLATIONS)
else:
    print("  No mi18n key to add")
